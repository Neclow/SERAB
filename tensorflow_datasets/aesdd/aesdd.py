...
_DESCRIPTION = """
The Acted Emotional Speech Dynamic Database (AESDD) is a publically available speech emotion recognition dataset.

It contains utterances of acted emotional speech in the Greek language, with 5 emotions: anger, disgust, fear, happiness, and sadness.
N.B.
The file sadness/s05 (3).wav is currently ignored, as triggering a runtime error ('No 'data' chunk marker'.).
"""
...
def parse_speaker_id(name):
    return str(int(re.search(r'\((\d+)\)', name).group()[1:-1]))
...
class Aesdd(tfds.core.GeneratorBasedBuilder):
    """DatasetBuilder for aesdd dataset."""
    ...
    manual_dir should contain the file Acted\ Emotional\ Speech\ Dynamic\ Database.zip.
    """

    def _info(self) -> tfds.core.DatasetInfo:
        """Returns the dataset metadata."""
        # TODO(aesdd): Specifies the tfds.core.DatasetInfo object
    ...
    def _split_generators(self, dl_manager: tfds.download.DownloadManager):
        ...
        if not tf.io.gfile.exists(zip_path):
            raise AssertionError(
                'AESDD requires manual download of the data. Please download '
